function [Vsav,Wsav]=AlievPanfilov1D_RK_ring(BCL,ncyc,extra,ncells,iscyclic,flagmovie)
...
stimgeo=false(X,1);
buffergeo=false(X,1);
stimgeo(3:7)=true; % indices of cells where external stimulus is felt
buffergeo(end-2:end)=true;
buffergeo(1:3)=true;

% Model parameters
dt=0.0005; % AU, time step for finite differences solver
...
stimdur=2; % UA, duration of stimulus
Va=0.3; % AU, value for V when cell is stimulated
